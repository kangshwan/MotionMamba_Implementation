...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_cls_token:   # True
            if self.use_double_cls_token:   # False
                ...
                if self.use_middle_cls_token:   # True
                    # (B, 1, embed_dim)
        ...
        if not self.if_bidirectional:
            ...
            for i in range(len(self.layers) // 2):
                if self.if_rope:
                    ...
                    if residual is not None and self.if_rope_residual:
                        residual = self.rope(residual)
                # residual_f: hidden_states
                hidden_states_f, residual_f = self.layers[i * 2](
                    hidden_states, residual, inference_params=inference_params
                )
                # residual_b: hidden_states.flip([1])
        ...
        if not self.fused_add_norm:
            ...
            # fused_add_norm_fn에서 residual 값이 더해지는 것을 확인했다. 타고 넘어가다보면 알 수 있다.
            # mamba-1p1p1\mamba_ssm\ops\selective_scan_interface.py
